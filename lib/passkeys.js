...
export function passkeyConfig(request) {
  ...
  // The WebAuthn RP ID must be the exact domain currently open in the
  // browser. Resolve it from Vercel's request headers instead of trusting a
  // manually typed environment value (which can contain a hidden space or be
  // a different preview domain).
  const requestOrigin = new URL(request.url).origin;
  const origin = host
    ? `${forwardedProtocol || new URL(requestOrigin).protocol.replace(":", "") || "https"}://${host}`
    : requestOrigin;
